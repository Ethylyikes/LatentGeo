...
def _patch_qwen2_vl_forward() -> bool:
    ...
    if isinstance(attn_map, dict) and "flash_attention_2" in attn_map:
        ...
        return True
    # Fallback to the bundled implementation if HF build lacks the class
    try:
        import latent_models.transformers.latent_modeling_qwen2_5_vl as latent_qwen
        if hasattr(latent_qwen, "Qwen2VLFlashAttention2"):
            setattr(m, "Qwen2VLFlashAttention2", latent_qwen.Qwen2VLFlashAttention2)
            latent_qwen.Qwen2VLFlashAttention2.forward = qwen2_vl_attn_forward
            if isinstance(attn_map, dict):
                attn_map["flash_attention_2"] = latent_qwen.Qwen2VLFlashAttention2
            return True
    except Exception:
        pass
    return False
...
def _patch_qwen2_5_vl_forward() -> bool:
    ...
    for name in ("Qwen2_5_VLFlashAttention2",):
        if hasattr(m, name):
            ...
            return True
    # 2b) If the HF module is missing the class, try to import the bundled implementation
    # and register it onto the HF module, so downstream imports succeed.
    try:
        import latent_models.transformers.latent_modeling_qwen2_5_vl as latent_qwen
        if hasattr(latent_qwen, "Qwen2_5_VLFlashAttention2"):
            setattr(m, "Qwen2_5_VLFlashAttention2", latent_qwen.Qwen2_5_VLFlashAttention2)
            latent_qwen.Qwen2_5_VLFlashAttention2.forward = qwen2_vl_attn_forward
            # Also update registry if present.
            attn_map = getattr(m, "QWEN2_5_VL_ATTENTION_CLASSES", None)
            if isinstance(attn_map, dict):
                attn_map["flash_attention_2"] = latent_qwen.Qwen2_5_VLFlashAttention2
            return True
    except Exception:
        pass
...
def apply_ulysses_patch(model_type: str) -> None:
    """
    Make flash-attn compatible with Ulysses sharding.

    - LLaMA-like backbones: overwrite the registry entry for FA2.
    - Qwen2-VL / Qwen2.5-VL: patch the concrete FA2 attention class if present,
      using version-tolerant lookups to avoid ImportError on different HF drops.
    """
    if model_type in ("llama", "gemma", "gemma2", "mistral", "qwen2", "qwen3", "qwen3_moe"):
        ALL_ATTENTION_FUNCTIONS["flash_attention_2"] = flash_attention_forward
        return
    ...
    if model_type == "qwen2_vl":
        if not _patch_qwen2_vl_forward():
            print(
                "[Latent monkey_patch] Warning: failed to patch Qwen2-VL FA2 forward; "
                "class/registry not found in this transformers build.",
                file=sys.stderr,
            )
        return
    ...
    if model_type == "qwen2_5_vl":
        if not _patch_qwen2_5_vl_forward():
            print(
                "[Latent monkey_patch] Warning: failed to patch Qwen2.5-VL FA2 forward; "
                "class/registry not found in this transformers build.",
                file=sys.stderr,
            )
        return

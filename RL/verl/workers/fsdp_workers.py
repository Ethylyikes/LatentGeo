...
"""
import latent_rl_patch  # noqa: F401
...
from examples.reward_function.latent_reward_function import extract_and_check as easyr1_latent_extract_and_check
from examples.reward_function.latent_reward_function import extract_and_check_api as easyr1_latent_extract_and_check_api
from examples.reward_function.latent_reward_function import rule_then_api_batch_judge
from examples.reward_function.wzh_reward_function import rule_then_api_batch_judge_wzh
...
class FSDPWorker(Worker):
    def __init__(
        ...
        if self._is_rollout and "api" in self.config.rule_based_judge.judge_function_name:
            ...
            if api_name in ['deepseek-chat', 'deepseek', 'deepseek-reasoner']:
                ...
                self.api_client = build_openai_client()
            else:
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor or self._is_critic:
            ...
            total_steps = max(1, optim_config.training_steps)
            if optim_config.lr_warmup_ratio > 0:
                num_warmup_steps = int(math.ceil(optim_config.lr_warmup_ratio * total_steps))
                # keep at least one warmup step and leave room for decay
                if num_warmup_steps >= total_steps:
                    num_warmup_steps = max(1, total_steps - 1)
            else:
                num_warmup_steps = 0

            sched_total_steps = max(total_steps, num_warmup_steps + 1)
            ...
            if optim_config.warmup_style == "cosine":
                ...
                # default: linear warmup to a flat lr
    ...
    def _build_rollout(self) -> None:
        ...
        if self.config.rollout.sampling_strategy in ["latent"]:
            self.hash_server = ray.get_actor(self.config.rollout.latent.hash_server_name)
        else:
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            if mode == "test":
                ...
                if self.config.rollout.sampling_strategy in ["latent"]:
                    output = self.rollout.generate_sequences_latent(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
            elif mode == "train_rl_gen":
                if self.config.rollout.sampling_strategy == "greedy":
                    output = self.rollout.generate_sequences(prompts=prompts)
                elif self.config.rollout.sampling_strategy in ["latent"]:
                    output = self.rollout.generate_sequences_latent(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
            else:
    ...
    def _get_rule_then_api_judge_fn(self):
        """Load configured rule_then_api judge (old/new) once and cache it."""
        if getattr(self, "_cached_rule_then_api_fn", None) is None:
            judge_cfg = getattr(self, "config", None).rule_based_judge if hasattr(self, "config") else None
            path = getattr(judge_cfg, "judge_function", None) if judge_cfg else None
            name = getattr(judge_cfg, "judge_function_name", None) if judge_cfg else None
            if path and name and os.path.exists(path):
                try:
                    spec = importlib.util.spec_from_file_location("custom_rule_based_judge_fn_worker", path)
                    module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(module)
                    self._cached_rule_then_api_fn = getattr(module, name, None)
                except Exception as exc:
                    print(f"[judge] failed to load {path}:{name}, fallback to defaults ({exc})")
            if getattr(self, "_cached_rule_then_api_fn", None) is None:
                if name == "rule_then_api_batch_judge":
                    self._cached_rule_then_api_fn = rule_then_api_batch_judge
                elif name == "rule_then_api_batch_judge_wzh":
                    self._cached_rule_then_api_fn = rule_then_api_batch_judge_wzh
        return getattr(self, "_cached_rule_then_api_fn", None)
    ...
    def compute_rule_based_judge(self, data: DataProto):
        ...
        if self.config.rule_based_judge.judge_function_name in ["rule_then_api_batch_judge", "rule_then_api_batch_judge_wzh"]:
            for i in range(len(data)):
                valid_response_ids = response_ids[i][: response_length[i]]
                response_str = self.tokenizer.decode(valid_response_ids, skip_special_tokens=True)
                response_strs.append(response_str)
            labels = data.non_tensor_batch.get("label")
            answer_labels = data.non_tensor_batch.get("answer_label")
            ...
            if answer_labels is not None:
                try:
                    ...
                    pass
            judge_fn = self._get_rule_then_api_judge_fn()
            if judge_fn is None:
            ...
            if answer_labels is not None:
                judge_kwargs["answer_labels"] = answer_labels
            try:
                correctness = judge_fn(**judge_kwargs)
            except TypeError:
                judge_kwargs.pop("answer_labels", None)
                correctness = judge_fn(**judge_kwargs)
        else:
            for i in tqdm(range(len(data)), desc="Rule-based judge", position=position, disable=self.rank != 0):
                ...
                if self.config.rule_based_judge.judge_function == "./examples/reward_function/latent_reward_function.py:extract_and_check":
                    correctness.append(easyr1_latent_extract_and_check(response_str, ground_truth))
                elif self.config.rule_based_judge.judge_function == "./examples/reward_function/latent_reward_function.py:extract_and_check_api":
                    correctness.append(easyr1_latent_extract_and_check_api(question, response_str, ground_truth, self.api_client))
                else:
                    raise NotImplementedError(f"Rule-based judge function {self.config.rule_based_judge.judge_function} not supported.")

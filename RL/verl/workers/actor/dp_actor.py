...
# Prefer robust shim to handle different transformers/flash-attn versions.
from .fa_shim import index_first_axis, pad_input, unpad_input
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        ...
        latent_poss = None
        latents = None
        if self.config.sampling_strategy == 'latent' and not self.config.ablate_latent:
            try:
                latent_poss = []
                start_env = os.getenv("ABS_VIS_START_ID") or os.getenv("LATENT_START_ID")
                end_env = os.getenv("ABS_VIS_END_ID") or os.getenv("LATENT_END_ID")
                if start_env is None or end_env is None:
                    raise ValueError("latent start/end ids are not set in env")
        ...
        if self.config.padding_free:
            ...
            # breakpoint()
            # Skip latent-specific kwargs to avoid TypeError on models that do not support them
            ...
            # Latent note:
            # We intentionally do NOT replace token log-probs with a Gaussian over latent embeddings.
            # Latent segments are masked out downstream (response_mask) so they won't contribute to GRPO/KL,
            # but the latent embeddings still influence subsequent token predictions via the forward pass.
    ...
    def _optimizer_step(self) -> tuple[torch.Tensor, float]:
        clip_scale = 1.0
        ...
        if not torch.isfinite(grad_norm):
            ...
            if grad_norm > 0:
                clip_scale = float(min(1.0, self.config.max_grad_norm / (grad_norm.item() + 1e-6)))
            self.actor_optimizer.step()

        self.actor_optimizer.zero_grad()
        return grad_norm, clip_scale
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        if self.config.sampling_strategy == "latent":
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                grad_norm, clip_scale = self._optimizer_step()
                append_to_dict(
                    metrics,
                    {
                        "actor/grad_norm": grad_norm.detach().item(),
                        "actor/clip_scale": clip_scale,
                        "actor/clip_applied": 1.0 if clip_scale < 1.0 else 0.0,
                    },
                )

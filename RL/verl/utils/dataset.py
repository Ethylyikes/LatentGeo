...
def dataset_name_from_path(data_path: str) -> str:
    ...
    if "latentgeo" in path_lower:
        return "latentgeo"
    if data_path.endswith(".json") or "merged" in data_path:
        return "LatentMath"
...
class RLHFDataset(Dataset, ImageProcessMixin):
    ...
    def __init__(
        ...
        self.debug_dataset = os.environ.get("LATENT_DATA_DEBUG", "0") == "1"
        self.force_rebuild_cache = os.environ.get("LATENT_FORCE_REBUILD_CACHE", "0") == "1"
        ...
        if os.path.isdir(data_path):
            ...
            # Respect the requested split when a single parquet/json file is given
        ...
        if self.filter_overlong_and_invalid_prompts:
            ...
            base_name = os.path.basename(os.path.normpath(self.data_path)) or "dataset"
            base_name = os.path.splitext(base_name)[0]
            path_hash = hashlib.md5(self.data_path.encode("utf-8")).hexdigest()[:8]
            cache_suffix = f"{base_name}-{data_split}-{path_hash}"
            cache_dir = os.path.join("./examples/dataset_valid_ids", dataset_name, cache_suffix)
            os.makedirs(cache_dir, exist_ok=True)
            cache_file = os.path.join(cache_dir, "valid_ids.txt")
            self._valid_ids_cache_path = cache_file
            #pdb.set_trace()
            use_cache = os.path.exists(cache_file) and not self.force_rebuild_cache
            if use_cache:
                ...
                if not valid_ids:
                    if self.debug_dataset:
                        print(f"[LatentDataDebug] cached valid ids empty, rebuild cache: {cache_file}")
                    use_cache = False
            if use_cache:
                if self.debug_dataset:
                    print(f"[LatentDataDebug] use cached valid ids: {len(valid_ids)} from {cache_file}")
                # Safety check: make sure ids are within range
                max_id = max(valid_ids) if valid_ids else -1
                if max_id >= len(self.dataset):
                    if self.debug_dataset:
                        print(
                            "[LatentDataDebug] cached ids out of range, rebuild cache: "
                            f"max_id={max_id}, dataset_len={len(self.dataset)}"
                        )
                    use_cache = False
                else:
                    self.dataset = self.dataset.select(valid_ids)
            else:
                if self.force_rebuild_cache and os.path.exists(cache_file) and self.debug_dataset:
                    print(f"[LatentDataDebug] force rebuild cache, remove {cache_file}")
                    try:
            ...
            if self.debug_dataset:
                print(f"[LatentDataDebug] cached valid ids: {len(self.dataset)}")
    ...
    def _filter_overlong_and_invalid_prompts(self, example: Dict[str, Any]) -> bool:
        if 'geometry3k' not in self.data_path.lower() and 'latentgeo' not in self.data_path.lower():
            if 'Thyme-RL' in self.data_path:
                example = self._build_example(example, dataset_name="Thyme")
            elif dataset_name_from_path(self.data_path) == "LatentMath":
                example = self._build_example(example, dataset_name="LatentMath")
            else:
                raise NotImplementedError(f"Dataset {self.data_path} not supported yet.")
            if not example:
                if self.debug_dataset:
                    print("[LatentDataDebug] drop sample: _build_example returned empty.")
                return False
    ...
    def _build_example(self, example, dataset_name):
        ...
        if dataset_name == "Thyme":
            if not example["images"] or len(example["images"]) > 1:
                if self.debug_dataset:
                    print("[LatentDataDebug] drop Thyme sample: images missing or >1")
                return {}
        ...
        elif dataset_name == "LatentMath":
            # mathcanvas-style json with fields: text_input/image_input/image_output/aux_think/text_output/correct_answer
            img_path = example.get("image_input")
            if img_path is None:
                if self.debug_dataset:
                    print("[LatentDataDebug] drop sample: no image_input field")
                return {}
            if not os.path.isabs(img_path):
                # Optional explicit root override (e.g., when JSON stores "./data/..." but images reside elsewhere)
                root_override = os.environ.get("LATENT_IMG_ROOT")
                if root_override:
                    # strip leading "./" to avoid double dots
                    cleaned = img_path.lstrip("./")
                    img_path = os.path.join(root_override, cleaned)
                else:
                    # resolve relative to repo root of the dataset file
                    base_dir = os.path.dirname(self.data_path)
                    img_path = os.path.normpath(os.path.join(base_dir, img_path))
            if not os.path.exists(img_path):
                if self.debug_dataset:
                    print(f"[LatentDataDebug] drop sample: image path not found -> {img_path}")
                return {}
            try:
                img = Image.open(img_path)
            except Exception as e:
                if self.debug_dataset:
                    print(f"[LatentDataDebug] drop sample: failed to open image {img_path}, err={e}")
                return {}
            data["images"] = [img]
            data["problem"] = example.get("text_input", "")
            # priority: explicit correct_answer > boxed text_output > raw text_output
            correct = example.get("correct_answer")
            if correct is not None and str(correct).strip() != "":
                answer = str(correct).strip()
            else:
                text_out = example.get("text_output", "") or ""
                match = re.search(r"\\boxed\\{(.*?)\\}", text_out)
                if match:
                    answer = match.group(1).strip()
                else:
                    answer = text_out.strip()
            data["answer"] = answer
            # Preserve optional choice label for downstream reward computation.
            answer_label = example.get("answer_label")
            if answer_label is not None and str(answer_label).strip() != "":
                data["answer_label"] = str(answer_label).strip()
        return data
    ...
    def __getitem__(self, index):
        ...
        # Ensure optional answer_label is present for collate consistency.
        if "answer_label" not in example:
            example["answer_label"] = None
        ...
        # Preserve optional supervision attributes (e.g., latent/nolatent labels) for downstream rewards.
        if "label" in example:

...
class ConsoleLogger(Logger):
    def __init__(self, config: Dict[str, Any], log_path: str = "./training_logs") -> None:
        self.log_path = log_path
        self.exp_name = config["trainer"]["experiment_name"]
        header = "Config\n" + convert_dict_to_str(config)
        self._write_to_file(header, overwrite=False)
        print(header)
    ...
    def log(self, data: Dict[str, Any], step: int) -> None:
        ...
        self._write_to_file(msg, overwrite=False)

    def _write_to_file(self, text: str, overwrite: bool = False) -> None:
        mode = "w" if overwrite else "a"
        # Ensure the directory exists even when log_path has no trailing slash.
        os.makedirs(self.log_path, exist_ok=True)
        save_path = os.path.join(self.log_path, f"{self.exp_name}.log")
...
class TensorBoardLogger(Logger):
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        keys_keep = ["model.name", "trainer.batch_size", "data.dataset"]
        safe_cfg = {k: _tb_safe(flat_cfg[k]) for k in keys_keep if k in flat_cfg}

        self.writer.add_hparams(
            hparam_dict=safe_cfg,

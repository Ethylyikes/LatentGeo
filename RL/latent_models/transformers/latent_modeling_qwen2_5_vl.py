...
"""AVT patched Qwen2.5 VL modeling module.

NOTE:
Original upstream file used relative imports with three leading dots (e.g. ``from ...activations``)
because its canonical package path is ``transformers.models.qwen2_5_vl``. Here we import this
file via a different module path: ``avt.transformers.avt_modeling_qwen2_5_vl`` and then monkey
patch it into ``sys.modules['transformers.models.qwen2_5_vl.modeling_qwen2_5_vl']`` inside
``sitecustomize.py``. Under this alternate package path, triple-dot relative imports try to go
above the top-level package (``avt``) and trigger ``ImportError: attempted relative import beyond
top-level package`` in every worker process.

To make the module robust regardless of how it is imported, we replace those fragile relative
imports with absolute ``transformers.*`` imports below.
"""
...
try:
    ...
    # Older transformers versions don't expose this helper; default to False.
...
if is_flash_attn_available():
    try:
        from transformers.modeling_flash_attention_utils import flash_attn_varlen_func as _flash_attn_varlen_func
        flash_attn_varlen_func = _flash_attn_varlen_func
    except Exception:
        flash_attn_varlen_func = None
    try:
        from transformers.modeling_flash_attention_utils import apply_rotary_emb as _apply_rotary_emb
...
if is_flash_attn_available():
    try:
        from transformers.modeling_flash_attention_utils import _flash_attention_forward as _flash_attention_forward
    except Exception:
        _flash_attention_forward = None

# Fail fast with a clear message if flash attention is requested but kernels are missing
if is_flash_attn_available() and flash_attn_varlen_func is None:
    def flash_attn_varlen_func(*args, **kwargs):
        raise ImportError(
            "flash_attn_varlen_func is unavailable. Please ensure flash-attn is installed "
            "and matches the local transformers version."
        )

if is_flash_attn_available() and _flash_attention_forward is None:
    def _flash_attention_forward(*args, **kwargs):
        raise ImportError(
            "flash attention forward kernel is unavailable. Please ensure flash-attn is installed "
            "and matches the local transformers version."
        )
...
def _broadcast_rope_like(x: torch.Tensor, rope: torch.Tensor) -> torch.Tensor:
    """
    Make RoPE cos/sin tensors broadcastable to attention tensors.

    Shapes we need to support:
    - rope: (seq_len, dim)
    - rope: (batch, seq_len, dim)
    Target shape for q/k: (batch, seq_len, num_heads, dim)
    """
    if rope.dim() == 2:
        # (seq, dim) -> (1, seq, 1, dim)
        rope = rope.unsqueeze(0).unsqueeze(2)
    elif rope.dim() == 3:
        # (batch, seq, dim) -> (batch, seq, 1, dim)
        rope = rope.unsqueeze(2)
    while rope.dim() < x.dim():
        rope = rope.unsqueeze(0)
    return rope


# Newer transformers no longer expose apply_rotary_emb; fall back to a local implementation.
if apply_rotary_emb is None:
    def apply_rotary_emb(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
        cos = _broadcast_rope_like(x, cos)
        sin = _broadcast_rope_like(x, sin)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if latent_poss is not None and latents is not None:
            #breakpoint()
            inputs_embeds[0, latent_poss] = latents

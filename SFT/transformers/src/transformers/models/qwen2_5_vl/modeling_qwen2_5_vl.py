...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # Mirage 项目新增参数说明:
        # pixel_values_latent: 隐空间图像的像素值输入（Stage 1 用于对齐训练）
        # image_grid_thw_latent: 隐空间图像的网格信息（T, H, W）
        # image_out_mask: 隐空间 Token 的掩码
        # generate_mode: 是否处于生成模式（影响 Stage 2 的推理逻辑）
        # latent_hidden_states: 预先计算好的隐层状态（用于替换 Latent Token）
        ...
        if inputs_embeds is None:
            ...
            ### Stage 1 修改: 处理 Latent Image Embeddings
            # 如果提供了 latent 图像的像素值（pixel_values_latent），则计算其 embedding 并替换对应的 latent token
            if pixel_values_latent is not None:
                ...
                # 将计算得到的 latent image embeddings 填入 input_embeds 中对应的 latent_token_id 位置
            ...
            # 如果直接提供了 latent hidden states 且不在生成模式下，直接替换
        ...
        if self.config.stage == 'stage2' and not generate_mode:
            ...
            if latent_indices.numel() == 0:
                ...
                # [DDP 适配] 放宽 latent size 检查
                if max_n_latents != self.config.latent_size:
                    # 在 DDP 中可能存在不匹配，这里允许通过
                    pass
            ...
            for latent_i in range(max_n_latents):
                ...
                # [DDP 适配] 只有 hidden_states 有效时才进行替换
                if hidden_states is not None:
                    # 将当前生成的 hidden state 填入下一个 latent token 的位置

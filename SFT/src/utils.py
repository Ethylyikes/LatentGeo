...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="latentgeo", choices=["latentgeo"])
    ...
    parser.add_argument("--per_device_train_batch_size", type=int, default=1)  # Batch size per device
    ...
    parser.add_argument("--max_seq_length", type=int, default=2048, help="Maximum sequence length for SFTTrainer")
    parser.add_argument("--attn_implementation", type=str, default="flash_attention_2", help="Attention implementation to use: 'eager', 'sdpa', or 'flash_attention_2'")
...
def load_jsonl_dataset(jsonl_path):
    """
    加载数据集函数，支持 JSONL 和标准 JSON 列表格式。
    """
    with open(jsonl_path, "r", encoding="utf-8") as f:
        ...
        try:
            ...
            if not isinstance(data, list):
                # 如果是合法的 JSON 但不是列表（例如单个字典），将其包装为列表
                data = [data]
        except json.JSONDecodeError:
            # 如果加载失败，尝试作为 JSONL（逐行读取）加载
            f.seek(0)
...
def replace_visual_special_tokens(texts):
...
replace_visual_spectial_tokens = replace_visual_special_tokens
...
def mask_image_output_tokens(
    ...
    """
    创建一个 mask，标记所有位于 [start_token, ..., end_token] 闭区间内的 token。
    用于在计算 CE loss 时忽略这些 token (Latent Tokens)。
    """
    mask = torch.zeros_like(input_ids)
    
    # 查找 start_token 的位置
    start_indices = (input_ids == start_token).nonzero()
    
    # 如果没有找到 start_token，直接返回全 0 mask
    if start_indices.numel() == 0:
        return mask
        
    for idx in start_indices:
        batch_idx, token_idx = idx[0], idx[1]
        
        # 在该行中查找对应的 end_token
        # 假设 start 和 end 是配对出现的，且嵌套不交叉
        # 找到从 token_idx 开始的第一个 end_token
        end_indices_in_row = (input_ids[batch_idx, token_idx:] == end_token).nonzero()
        
        if end_indices_in_row.numel() > 0:
            # end_token 的实际位置
            end_token_idx = token_idx + end_indices_in_row[0].item()
            
            # 标记区间 [start, end] 为 1
            mask[batch_idx, token_idx : end_token_idx + 1] = 1
